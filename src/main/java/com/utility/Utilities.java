...
public class Utilities extends ExtentReporter {
	...
	public static java.sql.DriverManager sqlDriver ;
	...
	public static void type(By byLocator, String input, String FieldName) throws Exception {
		try {
			...
			if (platform.equalsIgnoreCase("Android") || platform.equalsIgnoreCase("BrowserStack")
				...
				ele.sendKeys(Decryptpassword.decrypt(input));
				}else{
					ele.sendKeys(input);
				}
...
//			logger.error(e);
	...
	public static boolean verifyElementEnabled(By byLocator, String str) throws Exception {
		...
		try {
			...
			ExtentReporter.extentLogger("checkElementPresent", "" + str + " is not Enabled");
			logger.info(str + " is not Enabled");
			return false;
		}
		return false;
	}
...
//					driver.activateApp();
	...
	public static void EnterNewPassword() throws Exception {
		if(verifyElementDisplayed(ProfilePage.objNewPassword)) {
			String randompassword = RandomPassword();
			Actions a = new Actions(DriverManager.getAppiumDriver());
			click(ProfilePage.objNewPassword, "New password Input field");
			if(verifyElementDisplayed(ProfilePage.objPinkySwearPopup)){
				waitForElementToBePresent(ProfilePage.objGotItButton, 20,"Got it Button");
				click(ProfilePage.objGotItButton, "Got it Button");
			}
			a.sendKeys(randompassword).perform();
			logger.info("Entered Random New Password");
			ExtentReporter.extentLoggerPass("", "Entered RandomNew Password");
			hideKeyboard();
			click(ProfilePage.objReEnter, "Re Enter password Input field");
			a.sendKeys(randompassword).perform();
			logger.info("RE Entered RandomNew Password");
			ExtentReporter.extentLoggerPass("", "Re Entered RandomNew Password");
			verifyElementPresentAndClick(ProfilePage.objConfirmButton,"Confirm Button");
		}
	}
	...
	public static void enterOTP() throws Exception {
		...
		Actions a = new Actions(DriverManager.getAppiumDriver());
		click(OnBoardingPage.objOTPInputField, "OTP Input field");
...
//			Connection con = sqlDriver.getConnection(prop.getproperty("dbHostUrl"), prop.getproperty("dbUserName"),
//					prop.getproperty("dbPassword"));
			Connection con = sqlDriver.getConnection(propertyFileReader("dbHostUrl","Login"), propertyFileReader("dbUserName","Login"),
					propertyFileReader("dbPassword","Login"));
